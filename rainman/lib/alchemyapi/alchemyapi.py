...
class AlchemyAPI:
    ...
    def __init__(self):
        ...
        It loads the API key from the environment variable ALCHEMY_API and configures the endpoints.
        """
        import os
        try:
            key = os.environ.get('ALCHEMY_API')
            if not key:
                print('API key not found in environment variable ALCHEMY_API.')
                sys.exit(0)
            if key == '':
                #The key file should't be blank
                print('API key appears to be blank, please run: export ALCHEMY_API=YOUR_KEY_HERE')
                ...
                print('It appears that the key is invalid. Please make sure the environment variable includes the API key, and it is the correct one.')
                sys.exit(0)
            else:
                #setup the key
                self.apikey = key
